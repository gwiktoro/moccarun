...
def moccatest(run_path, make_path=None, commit=None, ref_path=None, set_moccaini=None, dry_run=False, do_snapshot_test=None, moccarun_kwargs=dict(), **kwargs):
    ...
    logger.debug(f"Unknown arguments to moccatest(): {kwargs}")

    cmd_l = []  # list of commands

    run_path = Path(run_path)
    #cmd_l.append(f"mkdir -p {run_path}")
...
#    run_path = runs_dir / test_name
    #test_name = run_path.stem
    ...
    if ref_path is not None:
        myrun(f"cp {ref_path}/{{mocca.ini,mocca.slurm,*_nbody.dat}} {run_path}")
        # only change the job name if we copy mocca.slurm from ref_path
#        cmd_l.append(f'sed -i "s/#SBATCH -J .*/#SBATCH -J {test_name}/" {run_path}/mocca.slurm')
#        cmd_l.append(f'sed -i "s/#SBATCH --mail-user=.*/#SBATCH --mail-user={user_email}/" {run_path}/mocca.slurm')
...
#            cmd_l.append(f'(cd {run_path} && moccarun --wait)')
#            cmd_l.append(f'test_snapshots.py {do_snapshot_test} {run_path / "snapshot.dat"}')
#        else:
#            cmd_l.append(f'(cd {run_path} && moccarun)')

    #cmd = ' && '.join(cmd_l)
    if not dry_run:
        #print(cmd)
        #exit(0)
        moccarun(run_path=run_path, **moccarun_kwargs)
...
def parse_args():
    ...
    parser.add_argument('run_path', type=Path, help='path to the test directory')
    ...
    parser.add_argument('--moccarun-kwargs', type=json.loads, default={}, help="arguments to moccarun as a json string")
...
def main():
    ...
    logger.parent.setLevel(args.logLevel)  # set to parent so it's visible in submodules
    logger.debug(args)
    del args.logLevel  # not needed in other functions

    # Call the function to execute the bash script
    #moccatest(args.run_path, src_path=args.src_path, ref_path=args.ref_path, user_email=args.user_email, commit=args.commit, compile=args.compile, set_moccaini=args.set_moccaini, do_snapshot_test=args.do_snapshot_test, dry_run=args.dry_run, run=not args.do_not_run)
    moccatest(**vars(args))
